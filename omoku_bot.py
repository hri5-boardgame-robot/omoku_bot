...
class omoku_bot:
    ...
    def robot_setup(self):
        self.m = mujoco.MjModel.from_xml_path('./low_cost_robot/low_cost_robot.xml')
    ...
    def etc(self):
        self.positions = [1965,2102,1891,1042,2040,2400]
        self.gripper("test")
    ...
    def debug_mode(self):
        ...
        while True:
            cmd = input("Debug mode: check the current position by just pressing enter, to quit, press z")
    ...
    def move(self, mode,destination):

        destination = np.array(destination)
        target_ee_rot = R.from_euler('z', 0, degrees=True).as_matrix().flatten()
    ...
    def gripper(self, mode):
        ...
        current_position = self.robot.read_position()
        interpolated_positions = self.robot.get_interpolate_pose(current_position, pwm_values)
        for positions in interpolated_positions:
            positions = np.array(positions)
            self.robot.set_goal_pos(positions)
            current_qpos = self.r._pwm2pos(positions)
